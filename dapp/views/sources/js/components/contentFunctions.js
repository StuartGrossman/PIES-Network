var contentFunctions = (function(userObject, firebaseDataBase){
  ...
  var exitScreen = false; //global needed to watch screen events
  var tagEle = document.createElement('div'); tagEle.classList.add('btn', 'smallTag'); //This element will be cloned
  // device detection
  ...
  this.saveData = function(thisEle){
    // var ele = document.getElementById('contentResponseHolder');
    // // console.log(ele.childNodes[11].childNodes[3])/
    // var count = 11;
    // for(var i = 0; i < 4; i++){
    //   console.log(ele.childNodes[11].childNodes[3].childNodes[1].style.display)
    //
    //   ele.childNodes[count].childNodes[3].childNodes[1].style.display = 'none';
    //   count += 2
    // }
    if(thisEle.parentNode.parentNode.childNodes[3].childNodes[1].style.display == 'none'){
      thisEle.parentNode.parentNode.childNodes[3].childNodes[1].style.display = 'block'
    }else{
